...
Primary domain: **ownership**. Highest-value extraction:
**Garage-as-home, diegetic instrumentation, run/repair cadence**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ---------------------------------------
  Player intent            Prepare car for a run and recover it
  Screen state             garage/home base
  Input                    repair/upgrade/load vehicle
  Selection feedback       diegetic station feedback
  Mechanical operation     install/repair equipment
  Simulation consequence   survivability/capability changes
  Testing                  expedition
  Persistence              car condition/upgrades/quirks persist
...
an event, save/reload and application relaunch: car
condition/upgrades/quirks persist.
...
`Prepare car for a run and recover it` and reach `expedition` without
