# 18 --- Expeditions: A MudRunner Game

## Why this game is in the top 25

Primary domain: **preparation**. Highest-value extraction:
**Risk/loadout planning, contextual tools**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ -----------------------------------------
  Player intent            Plan an expedition loadout
  Screen state             mission + garage/loadout
  Input                    choose vehicle/equipment/support
  Selection feedback       mission requirement/capability feedback
  Mechanical operation     equip resources
  Simulation consequence   field options change
  Testing                  expedition
  Persistence              resource/progression state persists
...
Record exact path from modification to proof: expedition. Count
...
an event, save/reload and application relaunch: resource/progression
state persists.
...
`Plan an expedition loadout` and reach `expedition` without encountering
an inert control or losing the changed state.
