...
Primary domain: **world**. Highest-value extraction: **Grassroots world,
local economy, multiple disciplines**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ---------------------------------------------------
  Player intent            Turn a humble project into multi-discipline racer
  Screen state             home garage/open world
  Input                    install parts/work/earn
  Selection feedback       physical + economy feedback
  Mechanical operation     upgrade vehicle
  Simulation consequence   capability changes
  Testing                  drag/oval/circuit
  Persistence              world/car/progression persist
...
Record exact path from modification to proof: drag/oval/circuit. Count
...
an event, save/reload and application relaunch: world/car/progression
persist.

## WTRL implementation translation

**Canonical truth owner:** WTRLCore subsystem appropriate to `world`.
...
`Turn a humble project into multi-discipline racer` and reach
`drag/oval/circuit` without encountering an inert control or losing the
changed state.

## WTRL prototype to derive

Grassroots world, local economy, multiple disciplines.
