# 23 --- Hill Climb Racing 2

## Why this game is in the top 25

Primary domain: **mobile**. Highest-value extraction: **Control economy,
readable suspension and weight transfer**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ -----------------------------------------
  Player intent            Drive complex terrain with two controls
  Screen state             side-view race
  Input                    throttle/brake taps
  Selection feedback       vehicle pitch/suspension animation
  Mechanical operation     continuous driving input
  Simulation consequence   weight transfer/air attitude changes
  Testing                  finish/retry
  Persistence              upgrades/progression persist
...
Record exact path from modification to proof: finish/retry. Count
...
an event, save/reload and application relaunch: upgrades/progression
persist.
...
`Drive complex terrain with two controls` and reach `finish/retry`
without encountering an inert control or losing the changed state.

## WTRL prototype to derive

Control economy, readable suspension and weight transfer.
