# 05 --- Car Mechanic Simulator 2021

## Why this game is in the top 25

Primary domain: **service**. Highest-value extraction: **Readable repair
workflow, shopping list, contextual modes**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ---------------------------------------------
  Player intent            Diagnose and complete repair order
  Screen state             job/car in garage
  Input                    inspect/select mount/unmount
  Selection feedback       highlight, condition and task feedback
  Mechanical operation     remove/repair/buy/install
  Simulation consequence   job completion and vehicle condition update
  Testing                  examine/run/test
  Persistence              job/garage/economy persists
...
Record exact path from modification to proof: examine/run/test. Count
...
an event, save/reload and application relaunch: job/garage/economy
persists.
...
`Diagnose and complete repair order` and reach `examine/run/test`
without encountering an inert control or losing the changed state.

## WTRL prototype to derive

Readable repair workflow, shopping list, contextual modes.
