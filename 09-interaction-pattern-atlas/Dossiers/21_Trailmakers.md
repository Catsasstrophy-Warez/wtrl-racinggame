...
Primary domain: **construction**. Highest-value extraction: **Accessible
engineering construction, immediate iteration**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ---------------------------------------------
  Player intent            Prototype a vehicle and test it immediately
  Screen state             builder
  Input                    place blocks/components
  Selection feedback       snap/COM/function feedback
  Mechanical operation     construct machine
  Simulation consequence   aero/weight/function emerge
  Testing                  instant test
  Persistence              blueprint persists
...
Record exact path from modification to proof: instant test. Count
...
an event, save/reload and application relaunch: blueprint persists.
...
`Prototype a vehicle and test it immediately` and reach `instant test`
without encountering an inert control or losing the changed state.

## WTRL prototype to derive

Accessible engineering construction, immediate iteration.
