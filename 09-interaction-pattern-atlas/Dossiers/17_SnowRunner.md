...
Primary domain: **preparation**. Highest-value extraction: **Preparation
as gameplay, surface-specific setup**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ----------------------------------------------
  Player intent            Prepare truck for terrain/mission
  Screen state             garage/map/loadout
  Input                    choose tires/suspension/drivetrain/equipment
  Selection feedback       capability preview
  Mechanical operation     configure truck
  Simulation consequence   terrain capability changes
  Testing                  mission/recovery
  Persistence              fleet/world/progression persist
...
Record exact path from modification to proof: mission/recovery. Count
...
an event, save/reload and application relaunch: fleet/world/progression
persist.
...
`Prepare truck for terrain/mission` and reach `mission/recovery` without
encountering an inert control or losing the changed state.

## WTRL prototype to derive

Preparation as gameplay, surface-specific setup.
