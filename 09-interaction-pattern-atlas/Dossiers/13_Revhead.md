...
Primary domain: **career**. Highest-value extraction: **Grassroots
economy and experimentation**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ -------------------------------------------
  Player intent            Build a viable racer from available parts
  Screen state             garage/economy
  Input                    buy/swap/tune
  Selection feedback       fitment/performance feedback
  Mechanical operation     assemble vehicle
  Simulation consequence   performance/reliability change
  Testing                  test/race
  Persistence              inventory/car/career persist
...
Record exact path from modification to proof: test/race. Count
...
an event, save/reload and application relaunch: inventory/car/career
persist.
...
`Build a viable racer from available parts` and reach `test/race`
without encountering an inert control or losing the changed state.

## WTRL prototype to derive

Grassroots economy and experimentation.
