...
Primary domain: **ownership**. Highest-value extraction: **Persistent
assembly truth, first-start ritual, sparse diegetic UI**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ -----------------------------------------
  Player intent            Build and start a car
  Screen state             garage full of loose parts
  Input                    pick/place/fasten/fill
  Selection feedback       physical placement + tool feedback
  Mechanical operation     assemble systems and add fluids
  Simulation consequence   incorrect assembly can affect operation
  Testing                  first start/drive/inspection
  Persistence              built state and mistakes persist
...
Record exact path from modification to proof: first
start/drive/inspection. Count transitions/taps/clicks and identify
avoidable friction.

### S6 --- Persistence

Record what survives leaving the screen, returning to garage, restarting
an event, save/reload and application relaunch: built state and mistakes
persist.
...
`Build and start a car` and reach `first start/drive/inspection` without
encountering an inert control or losing the changed state.

## WTRL prototype to derive

Persistent assembly truth, first-start ritual, sparse diegetic UI.
