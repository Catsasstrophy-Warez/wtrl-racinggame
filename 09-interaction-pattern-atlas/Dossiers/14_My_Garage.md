...
Primary domain: **service**. Highest-value extraction: **Physical tools,
wear-to-handling causality**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ------------------------------------
  Player intent            Repair worn/damaged car physically
  Screen state             first-person garage
  Input                    grab tool/part and work
  Selection feedback       world-space feedback
  Mechanical operation     repair body/mechanical systems
  Simulation consequence   handling/condition changes
  Testing                  drive/test
  Persistence              vehicle/tool/world state persists
...
Record exact path from modification to proof: drive/test. Count
...
an event, save/reload and application relaunch: vehicle/tool/world state
persists.
...
`Repair worn/damaged car physically` and reach `drive/test` without
encountering an inert control or losing the changed state.

## WTRL prototype to derive

Physical tools, wear-to-handling causality.
