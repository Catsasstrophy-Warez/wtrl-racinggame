...
Primary domain: **replay**. Highest-value extraction: **Low-clutter
motion feedback, ghost comparison**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ ----------------------------------------
  Player intent            Practice line/yaw and compare attempts
  Screen state             minimal arena/event
  Input                    steer/throttle/brake
  Selection feedback       clean motion/score feedback
  Mechanical operation     drive
  Simulation consequence   line/yaw/score consequence
  Testing                  ghost/replay/retry
  Persistence              records/ghosts persist
...
Record exact path from modification to proof: ghost/replay/retry. Count
...
an event, save/reload and application relaunch: records/ghosts persist.

## WTRL implementation translation

**Canonical truth owner:** WTRLCore subsystem appropriate to `replay`.
...
`Practice line/yaw and compare attempts` and reach `ghost/replay/retry`
without encountering an inert control or losing the changed state.

## WTRL prototype to derive

Low-clutter motion feedback, ghost comparison.
