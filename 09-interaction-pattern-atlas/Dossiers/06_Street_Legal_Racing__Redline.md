# 06 --- Street Legal Racing: Redline

## Why this game is in the top 25

Primary domain: **career**. Highest-value extraction:
**Garage→race→break→repair loop**.

## Canonical interaction sequence

  Stage                    Observed/target pattern
  ------------------------ --------------------------------------
  Player intent            Build a street/race car and prove it
  Screen state             garage vehicle + parts
  Input                    buy/install/tune
  Selection feedback       vehicle/stat/fitment feedback
  Mechanical operation     swap mechanical parts
  Simulation consequence   performance/damage changes
  Testing                  street/drag/circuit race
  Persistence              career, car and damage persist
...
Record exact path from modification to proof: street/drag/circuit race.
...
an event, save/reload and application relaunch: career, car and damage
persist.

## WTRL implementation translation

**Canonical truth owner:** WTRLCore subsystem appropriate to `career`.
...
`Build a street/race car and prove it` and reach
`street/drag/circuit race` without encountering an inert control or
