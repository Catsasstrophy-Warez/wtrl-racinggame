...
Primary domain: **tradeoff**. Highest-value extraction: **Nonlinear
upgrade tradeoffs**.
...
## Canonical interaction sequence
  ...
  ------------------------ -----------------------------------
  Player intent            Prepare for contact race
  Screen state             garage/upgrades
  Input                    add performance or armor parts
  Selection feedback       stats/visual build change
  Mechanical operation     install package
  Simulation consequence   mass/durability/performance trade
  Testing                  race/contact/damage
  Persistence              career/build persist
...
Record exact path from modification to proof: race/contact/damage. Count
...
an event, save/reload and application relaunch: career/build persist.

## WTRL implementation translation

**Canonical truth owner:** WTRLCore subsystem appropriate to `tradeoff`.
...
`Prepare for contact race` and reach `race/contact/damage` without
encountering an inert control or losing the changed state.

## WTRL prototype to derive

Nonlinear upgrade tradeoffs.
